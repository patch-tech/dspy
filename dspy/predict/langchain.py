...
class LangChainPredict(Predict, Runnable):  # , RunnableBinding):
    ...
    def __init__(self, prompt, llm, **config):
        ...
        self.signature, self.output_field_key = self._build_signature(
            langchain_template
        )
        self.config = config
        self.reset()
    ...
    def _build_signature(self, template):
        gpt4T = dspy.OpenAI(
            model="gpt-4-1106-preview", max_tokens=4000, model_type="chat"
        )
        ...
        for k, v in inputs.items():
            v.finalize(
                k, infer_prefix(k)
            )  # TODO: Generate from the template at dspy.Predict(Template2Signature)
        ...
        return (
            Template(parts.essential_instructions, **inputs, **outputs),
            output_field_key,
        )
    ...
    def forward(self, **kwargs):
        ...
        pred = Prediction.from_completions(
            [{self.output_field_key: content}], signature=signature
        )
